...
import * as shengnianUIReact from 'shengnian-ui-react'
...
export default (Component, options = {}) => {
  ...
  // ----------------------------------------
  // detect components like: shengnianUIReact.H1
  const isTopLevelAPIProp = _.has(shengnianUIReact, constructorName)

  // detect sub components like: shengnianUIReact.Form.Field (ie FormField component)
  ...
  while (meta) {
    ...
    meta = _.get(shengnianUIReact, [meta.parent, '_meta'])
  }
  ...
  // find the apiPath in the shengnianUIReact object
  const isSubComponent = _.isFunction(_.get(shengnianUIReact, apiPath))
    ...
    if (_.has(_meta, 'parent')) {
      describe('parent', () => {
        it('matches some component name', () => {
          expect(_.map(shengnianUIReact, c => c.prototype.constructor.name)).to.contain(_meta.parent)
        })
      })
    }
    describe('type', () => {
  ...
  describe('className (common)', () => {
    it(`has the Shengnian UI className "${componentClassName}"`, () => {
